...
export function StatsBand({ stats }: StatsBandProps) {
  ...
  return (
    <section id="stats" className="border-y border-white/[0.07] py-10 md:py-12">
        ...
        <dl className="grid grid-cols-2 gap-8 md:grid-cols-4 md:gap-6">
          {items.map((item) => (
            <div key={item.label} className="text-center md:text-left">
              ...
              <dd>
                <p className="font-landing-display text-2xl font-semibold tracking-tight text-white md:text-3xl">
                  {item.value}
                </p>
